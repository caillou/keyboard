...
eventtap.new({ eventTypes.keyDown }, function(event)
  ...
  if keycode == spaceKeyCode then
    ...
    state.modified = hs.timer.absoluteTime()
    state.spaceFnUsed = false
    return
  end

  if state.modified then
    if hs.timer.absoluteTime() - state.modified > 1.5e+8 then
      -- Letter key pressed after space has been held
      -- for 150ms. Assume that space is being used as
    ...
    table.insert(state.buffer, keycode)
    -- TODO TimerSet(NULL, 0, 200, spacefn_buffer_timer, pInfo);
    return
  end
...
--   typingTimer = hs.timer.doAfter(0.2, function()
--     state.isTyping = false
--   end)
-- end):start()
