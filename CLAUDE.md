...
- **`make test`**: runs `./lua_modules/bin/busted`. **`make test-watch`** runs it with `--watch` for sub-second iteration. **`make coverage`** runs the spec under luacov and prints the report (which Engine transitions the spec exercises); its `luacov.*.out` output is gitignored.
...
- **luacheck** — the linter (the eslint analog), scoped to logic only (unused vars, undefined globals, shadowing, unreachable code). All formatting and whitespace checks are switched off in `.luacheckrc` (notably `max_line_length = false` plus the W6xx family) so it never contradicts StyLua — the luacheck analog of `eslint-config-prettier`. `.luacheckrc` also declares `hs` as a read global, declares the deliberate module-level globals so they aren't flagged as undefined (`configWatcher`, `keyboardWatcher`, `keyUpDown`, `enableHotkeyForWindowsMatchingFilter`), excludes vendored/installed code from linting via `exclude_files` (`hammerspoon/Spoons/` and `lua_modules/`, mirroring `.styluaignore`), carves out `hs.window` as writable in `hammerspoon/windows.lua` (the documented pattern of attaching layout functions onto `hs.window`), and applies busted's standard set to `spec/`. Run via `make lint`. Installed via LuaRocks into the project-local `lua_modules/` tree, alongside busted. Note: luacheck must build against `lua@5.4` (keg-only, declared in the `Brewfile`) — its source reassigns generic-for loop variables, which Lua 5.5 rejects; `script/setup` handles this with `--lua-version=5.4`.
...
**shellcheck — the shell linter.** The fragile install path (`script/setup`, `script/update-emmylua`, with their documented `cliInstall` half-states and `PATH` repair logic) is shell, which StyLua and luacheck do not touch. shellcheck (`brew "shellcheck"`, defaults accepted, no config file — genuinely-needed suppressions are inline `# shellcheck disable=` directives) statically guards it. It is **folded into `make lint`**, which now runs `luacheck hammerspoon spec` followed by `shellcheck script/*`, so one command lints both Lua and shell and matches what the pre-commit hook runs.

**Enforcement: a lefthook pre-commit hook, deliberately no CI.** The quality gates above are only useful as a guardrail against agent-authored changes if something *runs* them. A root `lefthook.yml` defines a `pre-commit` stage that runs `make fmt-check`, `make lint` (Lua + shell), and `make test` as parallel commands at the commit boundary. The whole suite runs — no staged-file filtering — because it is sub-two-seconds on a repo this size, so lint-staged-style filtering would buy nothing. lefthook is installed via Homebrew (`brew "lefthook"`, matching the StyLua/shellcheck pattern — no npm, no Python runtime) and wired by `script/setup`'s idempotent `lefthook install` after `brew bundle`, so enforcement travels with a fresh clone needing no manual git-hook step. `git commit --no-verify` bypasses the hook (lefthook default) for an intentional work-in-progress commit. **CI is deliberately not added**: the workflow is direct commits to `master`, so a push-triggered CI run reports on a change that has *already* landed — it notifies but cannot prevent — whereas the commit boundary is the exact point a change is about to be recorded. Rationale recorded in full in `docs/adr/0002-enforce-checks-via-lefthook-not-ci.md`.

**Engine type annotations (engine-only, moderate diagnostics).** `space-fn-engine.lua` carries EmmyLua annotations (its Event tokens and Action tokens are each a `---@enum`; the payload-carrying action objects, the `Remap`/`KeyPayload` shapes, and the keymap entry are `---@class`/`---@alias`; `M.new`/`M:advance` carry `---@param`/`---@return`) — so the stringly-typed Event-token / Action-token protocol (see the `CONTEXT.md` glossary) is a checked, autocompleted type and a typo like `EventToken.spaceDwon` shows as an error instead of silently falling through to a default. The annotations are documentation-only: they add no runtime behavior, and because the enums are `local` to the Engine the Adapter still passes bare token strings across the boundary — engine-only scope by design, *not* a type-checked Adapter. The Engine is the right and only target: its public surface *is* its type, and the contract is stable and spec-pinned, so annotation-drift risk is lowest and payoff highest. The Adapter (`space-fn.lua`) and `windows.lua` are deliberately **not** annotated — the Adapter is intentionally thin and its bugs surface obviously at runtime (`docs/adr/0001-keep-space-fn-adapter-thin.md`), so annotating it earns little. LuaLS diagnostics (in `.vscode/settings.json`) are set to a **moderate** level via `Lua.diagnostics.neededFileStatus`, with `undefined-field` raised to `Error` under `Lua.diagnostics.severity` specifically so a mistyped token surfaces as a red error rather than a silent default. Strict global mode is avoided because it floods false positives against the Engine's `setmetatable({}, { __index = M })` object pattern. This rationale lives here in CLAUDE.md rather than in an ADR because annotations are trivially reversible (comments plus one editor setting, no behavior change), so they do not carry the lock-in that warrants an ADR.
