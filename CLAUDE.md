...
**Engine type annotations (engine-only, moderate diagnostics).** The plan is to add EmmyLua annotations (`---@enum` for the Event and Action tokens, `---@class` for action-payload and keymap-entry shapes, `---@param`/`---@return` on `M.new`/`M:advance`) to `space-fn-engine.lua` only — so the stringly-typed Event-token / Action-token protocol (see the `CONTEXT.md` glossary) becomes a checked, autocompleted type and a typo like `'space-dwon'` is underlined instead of silently falling through to a default. The Engine is the right and only target: its public surface *is* its type, and the contract is stable and spec-pinned, so annotation-drift risk is lowest and payoff highest. The Adapter (`space-fn.lua`) and `windows.lua` are deliberately **not** annotated — the Adapter is intentionally thin and its bugs surface obviously at runtime (`docs/adr/0001-keep-space-fn-adapter-thin.md`), so annotating it earns little. LuaLS diagnostics are set to a **moderate** level — enough to flag a bad token string or an undefined field, but not the strictest setting, which floods false positives against the Engine's `setmetatable({}, { __index = M })` object pattern. This rationale lives here in CLAUDE.md rather than in an ADR because annotations are trivially reversible (comments plus one editor setting, no behavior change), so they do not carry the lock-in that warrants an ADR.
